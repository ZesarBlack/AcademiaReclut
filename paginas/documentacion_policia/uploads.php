...
class Documentos
{
	...
	public function subirArchivo($cuip,$archivo,$temp_archivo)
	{
		...
		require '../../requires/conexion.php';
			...
			$response = file_get_contents("https://172.18.0.28/swebAcademia/?pass=c3s4RM494p&user=cesar&cuip=".$cuip, true, stream_context_create($arrContextOptions));
	...
	require '../../requires/conexion.php';
		...
		$response = file_get_contents("https://172.18.0.28/swebAcademia/?pass=c3s4RM494p&user=cesar&cuip=".$curp, true, stream_context_create($arrContextOptions));
		$policia = json_decode($response);
		//var_dump($policia);
