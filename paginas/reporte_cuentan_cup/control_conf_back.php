...
class ControlConf
{
  public function obtenerConfianza()
  {
    ...
    $query_conf="SELECT * FROM reporte_cuentan_cup";
    //$conn->query("SET CHARACTER SET 'utf8'");
    $resultado_conf=$conn->query($query_conf);
    while ($datos=mysqli_fetch_row($resultado_conf)) {

    $response = file_get_contents("http://172.18.0.28/swebAcademia/?pass=c3s4RM494p&user=cesar&control=".$datos[1], true, stream_context_create($arrContextOptions));
  ...
  public function actualizarCorte($control, $estatus)
  {
    include '../../requires/conexion.php';
    $query_act="INSERT INTO reporte_tienen_cup(estatus_cup, num_control) values ('$estatus', '$control')";
...
if (isset($_POST["control"]) && isset($_POST["estatus"])) {
  //echo "ok";
  $control->actualizarCorte($_POST["control"],$_POST["estatus"]);
}
